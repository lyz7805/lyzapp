  ...
  <el-row>
    <el-col :span="24">
      <div class="el-button el-button--plain el-button--mini">
        ...
        <el-button type="warning" icon="delete" size="mini" @click="$store.commit('setScbom', [])">清空数据</el-button>
      </div>
    ...
    <el-table :data="$store.state.scbom" stripe border max-height="500" id="wstable">
    ...
    data () {
      return {
        ...
        fileColumn: [
          ...
          { prop: '单位用量', label: '单位用量', sortable: true, align: 'right' }
        ]
      }
    },
    created () {
      ...
      submitUpload (evt) {
        if (evt !== undefined) {
          ...
          reader.onload = (e) => {
            ...
            this.$store.commit('setScbom', XLSX.utils.sheet_to_json(worksheet))
          }
